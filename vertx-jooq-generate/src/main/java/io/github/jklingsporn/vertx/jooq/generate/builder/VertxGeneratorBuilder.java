...
public class VertxGeneratorBuilder {
    ...
    static class ExecutionStepImpl implements ExecutionStep {
      ...
      public DIStep withR2dbcDriver() {
        base.setRenderDAOExtendsDelegate(AbstractVertxDAO.class::getName);// to change
        base.addWriteExtraDataDelegate((schema, writerGen) -> {
          ComponentBasedVertxGenerator.logger.info("Generate RowMappers ... ");
          String mappersSubPackage = base.getActiveGenerator().getVertxGeneratorStrategy().getRowMappersSubPackage();
          File moduleFile = base.generateTargetFile(schema, ".tables." + mappersSubPackage, "RowMappers.java");
        ...
        switch(base.apiType){
          case CLASSIC:
            return new DIStepImpl(base
                                          .setWriteDAOImportsDelegate(base.writeDAOImportsDelegate.andThen(out -> out.println("import io.github.jklingsporn.vertx.jooq.classic.r2dbc.R2dbcClassicQueryExecutor;")))
                                          .setRenderQueryExecutorDelegate((rType, pType, tType) -> String.format("R2dbcClassicQueryExecutor<%s,%s,%s>", rType, pType, tType))
                                          .setWriteConstructorDelegate((out, className, tableIdentifier, tableRecord, pType, tType,schema) -> {
                                            /*
                                             * pType = foo.bar.pojos.Somepojo
                                             * -------^-need--^---------------
                                            ...
                                            out.tab(1).javadoc("@param configuration Used for rendering, so only SQLDialect must be set and must be one of the POSTGREs types.\n" +
                                                                       "@param delegate A configured AsyncSQLClient that is used for query execution");
                                            out.tab(1).println("public %s(%s%s configuration) {", className, base.namedInjectionStrategy.apply(schema),Configuration.class);
                                            out.tab(2).println("super(%s, %s.class, new %s(configuration,%s.class, %s::new));", tableIdentifier, pType, base.renderQueryExecutor(tableRecord, pType, tType),pType,pType);
                                            out.tab(1).println("}");
                                          })
